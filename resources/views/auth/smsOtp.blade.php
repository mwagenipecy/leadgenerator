...
<head>
    ...
    <title>{{ __('auth.phone_otp_verification') }} - Fanikisha Market place</title>
...
@php
    ...
    $authSideImageValue = \App\Models\SystemSetting::getValue('auth_side_image_otp_' . app()->getLocale())
...
<!-- Left Side - Welcome Content (Hidden on mobile) -->
  <div class="hidden lg:flex lg:w-1/2 relative overflow-hidden h-screen" style="background-image: url('{{ $authSideImage }}'); background-size: cover; background-position: center; background-repeat: no-repeat;">
        ...
        <div class="text-center mb-8">
            ...
            <h2 class="text-2xl lg:text-3xl font-bold text-gray-900 mb-2">{{ __('auth.phone_verification') }}</h2>
            <p class="text-gray-600">{{ __('auth.otp_sent_to') }}</p>
        @php
            $phone = $user->phone;
            $maskedPhone = substr($phone, 0, 4) . str_repeat('*', strlen($phone) - 7) . substr($phone, -2);
        @endphp

        <p class="text-gray-800 font-medium">{{ $maskedPhone }}</p>
        ...
        <!-- Powered By -->
        <div class="mt-6 text-center">
            <p class="text-xs text-gray-400">Powered by CreditInfo</p>
        </div>
                </div>
            </div>
        </div>
