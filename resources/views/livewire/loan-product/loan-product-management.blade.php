<div class="w-full">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-4xl font-bold text-black mb-2">{{ __('navigation.loan_products') }}</h1>
                <p class="text-gray-600 text-lg">{{ __('admin.manage_loan_products_description') }}</p>
            </div>
            <div class="flex items-center space-x-3">
                <div class="flex items-center space-x-2 bg-sidebar-green-50 px-4 py-2 rounded-full">
                    ...
                    <span class="text-sm font-medium text-sidebar-green-light">{{ $stats['active'] ?? 0 }} {{ __('admin.active_products') }}</span>
                </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 border border-gray-100 hover:shadow-sm transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 mb-1">{{ __('admin.total_products') }}</p>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 border border-gray-100 hover:shadow-sm transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 mb-1">{{ __('admin.active_products') }}</p>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 border border-gray-100 hover:shadow-sm transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 mb-1">{{ __('admin.inactive_products') }}</p>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 border border-gray-100 hover:shadow-sm transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 mb-1">{{ __('dashboard.total_applications') }}</p>
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            ...
            <div class="flex items-center space-x-4">
                ...
                <select wire:model.live="employmentFilter" class="bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-sidebar-green focus:border-sidebar-green">
                    ...
                    <option value="employed">{{ __('admin.employed_only') }}</option>
                    <option value="unemployed">{{ __('admin.unemployed_self_employed') }}</option>
                </select>
            </div>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-black uppercase tracking-wider">{{ __('admin.product_name') }}</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-black uppercase tracking-wider">{{ __('admin.loan_amount_range') }}</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-black uppercase tracking-wider">{{ __('admin.interest_rate') }}</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-black uppercase tracking-wider">{{ __('admin.duration_range') }}</th>
                        <th class="px-6 py-4 text-center text-xs font-semibold text-black uppercase tracking-wider">{{ __('common.status') }}</th>
                        <th class="px-6 py-4 text-center text-xs font-semibold text-black uppercase tracking-wider">{{ __('common.actions') }}</th>
                    </tr>
                </thead>
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <h3 class="text-lg font-semibold text-black mb-2">{{ __('admin.no_products_found') }}</h3>
                                <p class="text-gray-500 mb-4">{{ __('admin.get_started_create_product') }}</p>
        ...
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                <div class="flex items-center mb-4">
                    ...
                    <h3 class="text-lg font-semibold text-black">{{ __('admin.delete_product') }}</h3>
                </div>
                <p class="text-gray-600 mb-6">{{ __('admin.confirm_delete_product') }} "{{ $selectedProductName }}"? {{ __('common.action_cannot_undone') }}</p>
        ...
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                <div class="flex items-center mb-4">
                    ...
                    <h3 class="text-lg font-semibold text-black">{{ __('admin.toggle_product_status') }}</h3>
                </div>
                <p class="text-gray-600 mb-6">{{ __('admin.confirm_change_status') }} "{{ $selectedProductName }}"?</p>
                <div class="flex justify-end space-x-3">
