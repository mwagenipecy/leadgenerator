...
<div>
    <div class="p-8">
        ...
        <!-- Lender Statistics Card -->
        @if($filterLender !== 'all' || $activeTab === 'bills')
                                ...
                                @forelse($applications as $application)
                                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div>
                                                <div class="text-sm font-bold text-black">{{ $application->application_number }}</div>
                                                <div class="text-xs text-gray-500">{{ $application->loan_purpose }}</div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="w-10 h-10 bg-gradient-to-br from-gray-700 to-black rounded-full flex items-center justify-center shadow-md">
                                                    <span class="text-white text-sm font-bold">{{ substr($application->first_name, 0, 1) }}{{ substr($application->last_name, 0, 1) }}</span>
                                                </div>
                                                <div class="ml-3">
                                                    <div class="text-sm font-medium text-black">{{ $application->first_name }} {{ $application->last_name }}</div>
                                                    <div class="text-xs text-gray-500">{{ $application->email }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-black">{{ $application->lender->company_name ?? 'N/A' }}</div>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-black">{{ $application->approved_at?->format('M d, Y') }}</div>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="px-6 py-12 text-center">
                                            ...
                                            <h4 class="text-lg font-semibold text-black mb-2">No Booked Applications</h4>
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                @forelse($bills as $bill)
                                    <tr class="hover:bg-gray-50 transition-colors duration-200 group">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div>
                                                <div class="text-sm font-bold text-black">{{ $bill->bill_number }}</div>
                                                <div class="text-xs text-gray-500">{{ $bill->created_at->format('M d, Y') }}</div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div>
                                                <div class="text-sm font-medium text-black">{{ $bill->application->application_number }}</div>
                                                <div class="text-xs text-gray-500">{{ $bill->application->first_name }} {{ $bill->application->last_name }}</div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="w-8 h-8 bg-gradient-to-br from-gray-700 to-black rounded-lg flex items-center justify-center shadow-sm">
                                                    <span class="text-white text-xs font-bold">{{ substr($bill->lender->company_name, 0, 2) }}</span>
                                                </div>
                                                <div class="ml-2">
                                                    <div class="text-sm font-medium text-black">{{ $bill->lender->company_name }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if($bill->commission_type === 'percentage')
                                                <div class="text-sm font-medium text-black">{{ $bill->commission_rate }}%</div>
                                                <div class="text-xs text-gray-500">TSh {{ number_format($bill->commission_amount) }}</div>
                                            @else
                                                <div class="text-sm font-medium text-black">Fixed</div>
                                                <div class="text-xs text-gray-500">TSh {{ number_format($bill->commission_amount) }}</div>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-bold text-black">TSh {{ number_format($bill->total_amount) }}</div>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="px-6 py-12 text-center">
                                            ...
                                            <h4 class="text-lg font-semibold text-black mb-2">No Commission Bills</h4>
                ...
                <div class="mb-6">
                    <div class="bg-gray-50 border border-gray-200 rounded-xl p-4">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <h4 class="text-sm font-semibold text-black">Bill Creation Summary</h4>
                                <p class="text-sm text-gray-700">You are about to create {{ count($selectedApplications) }} commission bills for the selected applications.</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    @if($showPaymentModal && $selectedBill)
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" wire:click.self="resetPaymentForm">
            <div class="relative top-20 mx-auto p-5 border w-11/12 max-w-3xl shadow-lg rounded-xl bg-white">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-2xl font-bold text-black">Record Payment</h3>
                ...
                <div class="mb-6 bg-gray-50 rounded-xl p-4">
                    <h4 class="font-semibold text-black mb-2">Bill: {{ $selectedBill->bill_number }}</h4>
    ...
    @if($showApplicationModal && $selectedApplication)
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" wire:click.self="closeApplicationModal">
            <div class="relative top-10 mx-auto p-5 border w-11/12 max-w-6xl shadow-lg rounded-xl bg-white">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-2xl font-bold text-black">Application Details</h3>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-1">
                        <div class="bg-gray-50 rounded-xl p-6">
                            <h4 class="text-lg font-semibold text-black mb-4">Applicant Information</h4>
                            <div class="space-y-3">
                                <div class="flex items-center space-x-3 mb-4">
                                    <div class="w-12 h-12 bg-gradient-to-br from-gray-700 to-black rounded-full flex items-center justify-center shadow-md">
                    ...
                    <div class="lg:col-span-1">
                        <div class="bg-gray-50 rounded-xl p-6">
                            <h4 class="text-lg font-semibold text-black mb-4">Loan Information</h4>
                    ...
                    <div class="lg:col-span-1">
                        ...
                        @if($selectedApplication->commissionBills->count() > 0)
                            <div class="bg-gray-50 rounded-xl p-6 mb-6">
                                <h4 class="text-lg font-semibold text-black mb-4">Commission Bills</h4>
                                <div class="space-y-3">
                        ...
                        @if($selectedApplication->documents->count() > 0)
                            <div class="bg-gray-50 rounded-xl p-6">
                                <h4 class="text-lg font-semibold text-black mb-4">Documents ({{ $selectedApplication->documents->count() }})</h4>
                                <div class="space-y-2">
    ...
    @if($showBillDetailsModal && $selectedBillForView)
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" wire:click.self="closeBillDetailsModal">
            <div class="relative top-10 mx-auto p-5 border w-11/12 max-w-5xl shadow-lg rounded-xl bg-white">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-2xl font-bold text-black">Bill Details - {{ $selectedBillForView->bill_number }}</h3>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <!-- Bill Information -->
                    <div class="bg-gray-50 rounded-xl p-6">
                        <h4 class="text-lg font-semibold text-black mb-4">Bill Information</h4>
                        <div class="space-y-3 text-sm">
                            <div class="flex justify-between py-2 border-b border-gray-200">
                    ...
                    <!-- Payment History -->
                    <div class="bg-gray-50 rounded-xl p-6">
                        <h4 class="text-lg font-semibold text-black mb-4">Payment History</h4>
                        ...
                        @if($selectedBillForView->payments->count() > 0)
                            <div class="space-y-3 mb-4">
                                @foreach($selectedBillForView->payments as $payment)
                                    <div class="bg-white rounded-lg p-3 border border-gray-200">
                                        <div class="flex items-center justify-between mb-2">
                                            <span class="text-sm font-medium text-black">TSh {{ number_format($payment->amount) }}</span>
                        ...
                        @else
                            <div class="text-center py-8">
                                ...
                                <h4 class="text-sm font-medium text-black mb-1">No Payments Recorded</h4>
                ...
                @if($selectedBillForView->notes)
                    <div class="mt-6 bg-gray-50 rounded-xl p-6">
                        <h4 class="text-lg font-semibold text-black mb-2">Notes</h4>
