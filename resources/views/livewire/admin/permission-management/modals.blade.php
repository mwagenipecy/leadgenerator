        ...
        <div class="relative top-20 mx-auto p-5 border w-11/12 max-w-lg shadow-lg rounded-3xl bg-white">
            ...
            <form wire:submit.prevent="createPermission" class="space-y-6">
                ...
                <div>
                    ...
                    @error('category') <span class="text-sidebar-green text-xs">{{ $message }}</span> @enderror
                </div>
            ...
            <div class="mb-6">
                ...
                @if($permissionToDelete->roles->count() > 0)
                    <div class="bg-sidebar-green-50 border border-sidebar-green-200 rounded-xl p-4 mb-4">
                        ...
                        <div class="mt-2">
                            <p class="text-sidebar-green-light text-xs font-medium mb-1">Assigned to roles:</p>
                            <div class="flex flex-wrap gap-1">
                                @foreach($permissionToDelete->roles->take(5) as $role)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-sidebar-green-100 text-sidebar-green-800">
                                        {{ $role->display_name }}
                                    </span>
                                @endforeach
                                @if($permissionToDelete->roles->count() > 5)
                                    <span class="text-xs text-sidebar-green">+{{ $permissionToDelete->roles->count() - 5 }} more</span>
                                @endif
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="flex items-center">
                                    @php
                                        $levelColor = match(true) {
                                            $roleLevel >= 80 => 'from-sidebar-green to-sidebar-green',
